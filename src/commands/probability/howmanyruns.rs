...
impl SlashSubCommand for HowManyRuns {
  ...
  async fn execute(&self, ctx: &Context, interaction: &ApplicationCommandInteraction, _: &Database) -> Result<()> {
    ...
    let items_required = probabilities.iter().map(|&p| attempts_required(p as f32 / 100_f32)).collect::<Vec<_>>();

    let mut probabilities_display = probabilities.iter()
      .map(|p| format!("{p}%"))
      .collect::<Vec<_>>();
    probabilities_display.insert(0, "Prob".to_string());

    let mut items_display = items_required.iter()
    ...
    let mut description = vec!["Items: The total amount of items to have an X% chance of at least 1 rare drop".to_string()];

    if let Some(items) = items_per_run {
      description.push(format!("Runs: The number of runs required, assuming **${items}** items per run"));
      let mut runs_required = items_required.iter()
    ...
    if let Some(time) = time_per_run {
      description.push(format!("Hours: The number of hours spent farming, assuming one run takes **${time}** minutes"));
      let mut time_required = items_required.iter()
        .map(|&n| {
          let formatted = format!("{:.1}", (n as f32 * time as f32 / (60_f32 * items_per_run.unwrap_or(1) as f32)));
