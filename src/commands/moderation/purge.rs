...
impl SlashCommand for Purge {
  ...
  async fn execute(&self, ctx: &Context, interaction: &ApplicationCommandInteraction, _: &Database) -> Result<()> {
    ...
    if
      count <= 100 && !bots_only && !humans_only && !must_have_embed && !must_have_image && !must_have_link &&
      !must_have_mention && after == 0 && from_author_id == 0 && exact.is_empty() && beginning.is_empty() && ending.is_empty()
    {
      ...
      interaction.edit_original_interaction_response(&ctx.http, |msg| msg.set_embed(embed)).await?;
    } else {
      // Custom purge
      ...
      let can_delete = |msg: &Message| -> bool {
        ...
        !(
          ...
          (!ending.is_empty()     && !content.ends_with(&ending)        )    // Only ends with phrase and doesn't
        )
      };
