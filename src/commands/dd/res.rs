...
impl SlashSubCommand for Res {
  ...
  async fn execute(&self, ctx: &Context, interaction: &ApplicationCommandInteraction, _: &Database) -> Result<()> {
    ...
    if resists.iter().any(|e| e.abs() > 35) {
      ...
      embed.color(Colors::Red).title("Reasonable numbers expected");
    ...
    embed_footer.text(format!(
      ...
      resists
        ...
        .collect::<Vec<String>>()
        .join(" "),
      if let Some(upgrade_count) = upgrades {
        format!("  |  Available upgrades: {upgrade_count}")
      } else {
        String::default()
      },
      if let Some(primary) = primary_stat {
        format!("  |  Main stat: {primary}")
      } else {
        String::default()
      },
      if let Some(secondary) = secondary_stat {
        format!("  |  Secondary stat: {secondary}")
      } else {
        String::default()
      }
    ));
